 ...
 */
static int
newnextarg (char **outbufref, const char *prompt, int size, int terminator)
{
  int status;
  char *buf;
...
int
newmlargt (char **outbufref, const char *prompt, int size)
{
              ...
              if (strncmp (buf, ffp->n_name, strlen (buf)) == 0)
                {
                  ...
                  if ((ffp + 1)->n_func == NULL
                  ...
                  else
                    {
                      ...
                      /* first scan down until we no longer match the current
                       * input */
                      lffp = (ffp + 1);
                      while ((lffp + 1)->n_func != NULL)
                        {
                          if (strncmp (buf, (lffp + 1)->n_name, strlen (buf))
                              != 0)
                            break;
                          ++lffp;
                        }

                      /* and now, attempt to partial complete the string, char
                       * at a time */
                      while (TRUE)
                        {
...
int
getstring (const char *prompt, char *buf, int nbuf, int eolchar)
{
