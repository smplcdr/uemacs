...
static int kbdflags; /* Saved keyboard fd flags.  */
...
ttopen (void)
{
  ...
  /* Base new settings on old ones -- do not change things
     we don't know about.  */
  ntermios = otermios;
  ...
  ntermios.c_lflag &= ~(ISIG | ICANON | XCASE | ECHO | ECHOE | ECHOK | ECHONL | NOFLSH
                        | TOSTOP | ECHOCTL | ECHOPRT | ECHOKE | FLUSHO | PENDIN | IEXTEN);

  /* One character, no timeout.  */
  ntermios.c_cc[VMIN] = 1;
  ntermios.c_cc[VTIME] = 0;
  tcsetattr (0, TCSADRAIN, &ntermios); /* Activate them.  */

  /* Provide a smaller terminal output buffer so that
     the type ahead detection works better (more often).  */
  setbuffer (stdout, &tobuf[0], TBUFSIZ);

  kbdflags = fcntl (0, F_GETFL, 0);
  kbdpoll = FALSE;
...
/* Flush terminal buffer. Does real work where the terminal output is buffered up.
   A no-operation on systems where byte at a time terminal I/O is done.  */
void
ttflush (void)
{
  /*
...
/* Read a character from the terminal, performing no editing and doing no echo at all.
...
ttgetc (void)
{
  ...
  if (count < expected)
    {
      ...
      n = read (STDIN_FILENO, buf + count, sizeof (buf) - count);
  ...
  bytes = utf8_to_unicode (buf, 0, pending, &c);

done:
  pending -= bytes;
  memmove (buf, buf + bytes, pending);
  return c;
}
