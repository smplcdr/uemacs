...
getkill (void)
{
  ...
  if (kbufh == NULL)
    /* No kill buffer -- just an empty string.  */
    return "";
...
#define BLOCK_SIZE 16 /* Line block chunk size.  */
  ...
  lp = malloc (offsetof (struct line, l_text) + size);
  if (lp == NULL)
...
lchange (int flag)
{
  ...
  if ((curbp->b_flag & BFCHG) == 0)
    {
      /* First change, so update mode lines.  */
      flag |= WFMODE;
      curbp->b_flag |= BFCHG;
    }
  wp = wheadp;
      ...
      while ((tmpc = *instr++ & 0xFF) != '\0')
        {
          ...
          if (status != TRUE)
            {
              mloutstr ("%Memory exhausted while inserting");
              return status;
            }
        }
    }
      ...
      while ((c = *ostr++) != '\0')
        {
          ...
          if (status != SUCCESS)
            {
              mloutstr ("%Memory exhausted while overwriting");
              return status;
            }
        }
    }
...
kdelete (void)
{
  struct kill *kp; /* Pointer to scan kill buffer chunk list.  */
...
kinsert (int c)
{
  struct kill *nchunk; /* Pointer to newly malloced chunk.  */

  /* Check to see if we need a new chunk.  */
  if (kused >= KBLOCK)
    {
      if ((nchunk = malloc (sizeof (*nchunk))) == NULL)
        return FAILURE;
      if (kbufh == NULL)
        {
...
yank (bool f, int n)
{
  ...
  if (kbufh == NULL)
    return SUCCESS; /* not an error, just nothing */
